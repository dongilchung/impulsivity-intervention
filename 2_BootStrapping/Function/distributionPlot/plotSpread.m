...
if ~isempty(varargin) && ~ischar(varargin{1}) && ~isstruct(varargin{1})
    ...
    opt.distributionMarkerSize = def.distributionMarkerSize;
    opt.xMode = def.xMode;
    opt.xyOri = def.xyOri;
    opt.categoryIdx = [];
...
if isnumeric(opt.distributionMarkerSize)
    opt.distributionMarkerSize = {opt.distributionMarkerSize};
end
...
if length(opt.distributionMarkerSize) == 1
    % expand
    opt.distributionMarkerSize = repmat(opt.distributionMarkerSize,nData,1);
elseif length(opt.distributionMarkerSize) ~= nData
    error('please submit one color per distribution (%i dist, %i colors)',nData,length(opt.distributionMarkerSize));
end
...
plotMarkerSize= repmat(opt.distributionMarkerSize(:),1,nCategories);
...
if nCategories > 1
    ...
    if ~any(strcmp('categoryMarkers',parserObj.UsingDefaults))
        ...
        plotMarkerSize = repmat(opt.categoryMarkerSize(:)',nData,1);
        categoryIsLabeled = true;
    end
            ...
            switch opt.xyOri
                case 'normal'
                    ph(iData,iCategory) = plot(ah,data(currentIdx,1),...
                        ...
                        'MarkerSize',plotMarkerSize{iData,iCategory},...
